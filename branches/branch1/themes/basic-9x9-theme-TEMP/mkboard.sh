...
convert  -size "$STONE_IMAGE_SIZE" xc:none -fill "$BLACK_STONE_COLOR" \
	 -draw "$DRAW_COMMAND"   "$BLACK_STONE_FILENAME"
convert  -size "$STONE_IMAGE_SIZE" xc:none -fill "$WHITE_STONE_COLOR" \
	 -draw "$DRAW_COMMAND"   "$WHITE_STONE_FILENAME"

DRAW_COMMAND="circle $((STONE_SIZE / 2 - 1)),$((STONE_SIZE / 2 - 1)) $((STONE_SIZE / 2)),$((STONE_SIZE * 3 / 8))"
convert  "$BLACK_STONE_FILENAME" -fill "$WHITE_STONE_COLOR" \
	 -draw "$DRAW_COMMAND"  "$BLACK_LAST_STONE_FILENAME"

DRAW_COMMAND="circle $((STONE_SIZE / 2 - 1)),$((STONE_SIZE / 2 - 1)) $((STONE_SIZE / 2)),$((STONE_SIZE * 3 / 8))"
convert  "$WHITE_STONE_FILENAME" -fill "$BLACK_STONE_COLOR" 
	 -draw "$DRAW_COMMAND"  "$WHITE_LAST_STONE_FILENAME"
...
convert  -size "$i"x"$j" \
         xc:none -fill "$BOARD_BACKGROUND_COLOR" \
...
#
# Note: Handicap stones not used at the moment as I don't know where
#	to place them...
#
i=3 #0
while [ "$i" -le "2" ]; do
